...
const begWord = {}, inWord = {}, space = {}, LF = '\n'.charCodeAt(0), LT = '<'.charCodeAt(0), GT = '>'.charCodeAt(0), EXCLAM = '!'.charCodeAt(0), QMARK = '?'.charCodeAt(0), SLASH = '/'.charCodeAt(0), OBRA = '['.charCodeAt(0), EQ = '='.charCodeAt(0), DASH = '-'.charCodeAt(0), entitiesByChar = {
    '&': 'amp',
    '<': 'lt',
    '>': 'gt',
    '"': 'quot',
...
(function () {
    ...
    for (let i = 0; i <= 9; i++)
        add(inWord, '0', i);
    for (let i = 0; i < 26; i++)
        add(begWord, 'aA', i), add(inWord, 'aA', i);
...
function builder(error) {
    ...
    return {
        push(tag) {
            if (elt.$cdata != null)
                throw error('cannot mix CDATA and children');
            if (elt.$value != null)
                throw error('cannot mix value and children');
            elt.$childCount++;
            const child = {
                $tag: tag,
                $parent: elt,
        ...
        pop(tag) {
            if (tag && tag !== elt.$tag)
                throw error('closing tag mismatch: expected ' + elt.$tag + ', got ' + tag);
            const parent = elt.$parent;
            if (!parent)
                throw error('too many closing tags');
            delete elt.$parent;
            // if elt does not have attributes, replace it by value in parent
            if (elt.$value !== undefined && !elt.$) {
                if (elt.$index === undefined)
                    parent[elt.$tag] = elt.$value;
                else
                    parent[elt.$tag][elt.$index] = elt.$value;
            }
            else {
                delete elt.$tag;
        ...
        attribute(atb, val) {
            elt.$ = elt.$ || {};
            if (elt.$[atb] != null)
                throw error('duplicate attribute: ' + atb);
            elt.$[atb] = val;
        },
        value(val) {
            if (elt.$cdata != null)
                throw error('cannot mix CDATA and value');
            if (elt.$childCount)
                throw error('cannot mix children and value');
            elt.$value = val;
        },
        cdata(val) {
            if (elt.$value != null)
                throw error('cannot mix value and CDATA');
            if (elt.$childCount)
                throw error('cannot mix children and CDATA');
            elt.$cdata = val;
        },
        getResult() {
            if (elt !== root)
                throw error('tag not closed: ' + elt.$tag);
            if (!root.$childCount)
                throw error('root tag not found');
            if (root.$childCount !== 1)
                throw error('too many elements at top level');
...
function parse(str, reviver) {
    let pos = str.indexOf('<');
    const len = str.length, bld = builder(error);
    function error(msg) {
    ...
    function eatSpaces() {
        while (space[str.charCodeAt(pos)])
            pos++;
    }
    function eat(ch) {
    ...
    function eatQuote() {
        const ch = str[pos];
        if (ch !== '"' && ch !== "'")
            throw error("expected quote, got '" + ch + "'");
        pos++;
        return ch;
    }
    function clean(str) {
        return str.replace(/&([^;]+);/g, function (s, ent) {
            ...
            if (ch)
                return ch;
            if (ent[0] !== '#')
                throw error('invalid entity: &' + ent + ';');
            ent = ent.substring(1);
            let radix = 10;
            if (ent[0] === 'x') {
                ent = ent.substring(1);
            ...
            if (isNaN(v))
                throw error('hex value expected, got ' + ent);
    ...
    while (pos < len) {
        ...
        if (begWord[ch]) {
            while (inWord[str.charCodeAt(pos)])
                pos++;
            ...
            while (true) {
                ...
                if (ch === SLASH) {
                    ...
                    if (pos < 0)
                        throw error('double quote missing');
    ...
    function escape(val) {
        return typeof val !== 'string'
            ...
            : val.replace(/([&<>"']|[^ -~\u00a1-\ud7ff\ue000-\ufffd])/g, function (ch) {
                ...
                while (hex.length < 2)
                    hex = '0' + hex;
                while (hex.length > 2 && hex.length < 4)
                    hex = '0' + hex;
                return '&#x' + hex + ';';
            });
    }
...
///
function normalizeNamespaces(obj) {
...
function stringify(elt, options) {
    options =
    ...
    if (keys.length !== 1)
        throw error('bad root element expected 1 child got ' + keys.length);
    strfy(elt[keys[0]], keys[0]);
    return fmt.getResult();
}
exports.stringify = stringify;
//# sourceMappingURL=js-xml.js.map